...
>
> **改訂 5(2026-07-30、収束ラウンド 1〜2)**: v2 本文が収束ラウンド 1(確定
> critical 1 / major 6 + minor 10)と収束ラウンド 2(確定 critical 2 — 同一欠陥で
> 統合 — / major 8 + minor 9)の指摘を反映して改訂された。主な変更: forward re-pin の
> 書込単位判定への訂正を経た**証明書手順 5 の二重定足数化**(原 pin proof の恒久併載 +
> 前進条件の ids ⊇ 単一化 — v2 §3.3)、外部検証の**信頼アンカー(検証手順 0)**新設と
> 帯外鍵レジストリへの一本化(名簿歴史チェーンは v2 §10-15 の将来オプションへ)、
> sweep 証跡の開始時刻基準 + sweep 局所 zero-error 条件(v2 §3.5/§3.8)、sweep・報告
> 配送の**送信側耐久化**(v2 §3.2/§3.8)、非 authority roster ノードの検証鍵 = roster
> レコード同梱と、構成拒否・報告生成規律の主語の roster データノード全体への拡張
> (v2 §3.2/§5.1)、origin 引退の接頭辞マスク化と un-retire(v2 §3.2/§3.5)。
> principles.md は T2 に再検討条件が追記され、登録簿規律が緊張表の登録簿行へ拡張
> された。裁定記録は v2 §11 収束ラウンド 1/2 表。
