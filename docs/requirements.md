...
- CRDT は状態収束のために利用し、文章編集系 CRDT は本プロジェクトのスコープ外とする
...
- MVP の `digest_hash` 検証は key_range 単位の周期チェックポイント方式を採用すること
  - 実装状況の系統分離（M-12 時点）: **frontier 報告の `digest_hash` は実 digest 化済み**
    （eventual store の M-7 root digest `sd2:<hex64>` を署名付きで束縛する）。ただし検知は
    「同一 Authority が**完全一致する同一 checkpoint HLC** に異なる digest 文字列を署名した」場合の
    等価比較に限定される——報告ごとに HLC をずらした split-view や HLC が一致しない一過性の分岐は
    検知対象外（CT 型 consistency proof は将来課題。`docs/ops-guide.md`「Equivocation / split-view
    検知」の限界一覧を参照）。
