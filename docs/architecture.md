...
graph TB
    ...
    CP -->|Raft log replication| CP2[Peer Control Plane]
```
...
system namespace に格納されたクラスタ全体の設定を管理します。ポリシー / Authority 定義の
変更は**内蔵 Raft コンセンサス**（`src/control_plane/raft/`）で合意されます。
かつての「承認カウントによる simple majority check」は廃止され、リーダーのログ複製が
唯一の合意経路です。

| コンポーネント | 場所 | 役割 |
|--------------|------|------|
| System Namespace | `src/control_plane/system_namespace.rs` | 配置ポリシーと Authority 定義の格納（Raft 状態機械の射影） |
| Consensus Facade | `src/control_plane/consensus.rs` | Raft ノードへの Clone 可能なハンドル（旧 API 互換の detached モード付き） |
| Raft Core | `src/control_plane/raft/core.rs` | 純粋・決定的な Raft 遷移（term / 投票 / ログ複製 / commit 前進） |
| Raft State Machine | `src/control_plane/raft/state_machine.rs` | コミット済みエントリの適用（ポリシーバージョン採番を含む） |
| Raft Storage | `src/control_plane/raft/storage.rs` | `currentTerm`/`votedFor` とスナップショット+ログの fail-stop 永続化 |
| Raft Transport | `src/network/raft_transport.rs` | `/api/internal/raft/*` HTTP RPC（Bearer トークン + bincode/JSON フォールバック） |
...
#### Control Plane Raft の設計

research（`research/topics/quorum-consensus.md`）の示唆どおり、制御プレーンの標準解として
対称 majority quorum + ランダム化選挙タイムアウトの素の Raft を採用しています
（Flexible Paxos の非対称 quorum や leaderless 系は Certified path の将来最適化素材に留める）。
スループットは重要でない（低頻度・小ログ）ため、実装は一貫して正しさ優先です。

**3 つの安全性規則**（論文 Figure 2 準拠）:

1. **選挙制限**: 投票者は `(lastLogTerm, lastLogIndex)` の辞書順比較で自ログ以上に
   新しい候補にのみ投票する（Leader Completeness の担保）。
2. **commit 規則（Figure 8）**: エントリは**現 term のリーダー**が過半数に複製したときのみ
   commit される。過去 term のエントリは現 term の no-op（当選時に必ず append）の commit を
   介して間接的にのみ commit される。
3. **term フェンシング**: 全 RPC が term を運び、小さい term は拒否・大きい term を見た側は
   即 follower 降格。stale リーダーは過半数 ack を得られず自動的にフェンシングされる。

加えて **prevote-lite ガード**（dissertation §4.2.3 の removed-server 規則）: 最小選挙
タイムアウト内に現リーダーから heartbeat を受けている投票者は、RequestVote を **term を
採用せずに**無視する。分断から復帰したノードの term 暴走が健全なリーダーを乱すのを防ぐ
（完全な prevote ではないため、復帰ノード起因の余分な選挙が最大 1 ラウンド発生し得る）。

**永続化不変条件**（fail-stop、ベストエフォート不可）:

- `currentTerm` / `votedFor` / ログエントリは、それに依存する応答（投票・append ack・
  proposal 受理）を返す**前に** fsync される。永続化に失敗した場合は応答そのものを
  返さない（`data_dir/raft/hard_state.json` と `log.json`）。
- スナップショットとログ tail は**単一ファイルに原子的に共永続化**されるため、
  コンパクション途中のクラッシュで「ログは削れたがスナップショットがない」状態は
  構造的に発生しない。
- `ASTEROIDB_PERSISTENCE=off` でも Raft 永続化は常時有効（選挙安全性の要件であり
  最適化ではない）。起動時に Raft 状態ファイルが破損していた場合は fail-stop
  （復旧手順は ops-guide 参照）。

**状態機械 = 複製コアのみ**: 複製されるのは (1) 配置ポリシー、(2) 手動 Authority 定義、
(3) 決定的な `version_counter` の 3 つ。ポリシーバージョンはコミット済みエントリの
**適用時**に counter から採番されるため、全ノードで同一の値に決定的に収束する
（propose 時採番は禁止）。一方、`recalculate_authorities` による自動生成 Authority、
`version_history`、namespace バージョンそのものはノード局所の導出値であり複製されない —
「同一ログ ⇒ 同一状態」は複製コアの射影についてのみ成立する。

**フェンシングのレイヤ分離**: Raft の term は**制御プレーン**の stale リーダーを
フェンシングする。`AckFrontierSet::fence_version`（(KeyRange, PolicyVersion) 対の
フェンシング）は **Authority プレーン**のポリシー世代交代を担う別レイヤであり、
互いの代用にはしない。依存は一方向のみ:
制御プレーン commit → namespace バージョン bump → `NodeRunner::detect_version_changes`
のポーリング → 旧バージョンの fence + `recalculate_authorities` + リバランス。
Raft の適用パスはこれらを直接呼ばない（二重実行の防止）。

**ブートストラップ**: 初回リーダーは自ノードのローカル複製コア全体を 1 個の `Bootstrap`
エントリとして提案する（reset-and-import、適用は冪等）。`version_floor` にリーダーの
ローカル namespace バージョンを入れることで、pre-Raft 状態からの移行でもポリシー
バージョンフェンシングの単調性が保たれる。起動時の catch-all Authority 定義シードは
**初回ブート時のみ**行われる（毎起動 bump による状態機械分岐の防止）。

**スコープ外**（意図的に実装しないもの）:

- 動的メンバー変更（joint consensus / single-server 変更）— 投票者集合は
  `ASTEROIDB_CONTROL_PLANE_NODES` による静的設定。変更手順は ops-guide 参照。
- チャンク式スナップショット転送 — InstallSnapshot は単一メッセージの lite 版
  （制御プレーン状態は小さい前提。コンパクション超過 follower の救済と移行時の
...
**分断時の挙動**: 過半数を失った側（少数派）では制御プレーンは「読める・書けない」—
ポリシー参照は可能だがポリシー更新は 503 / タイムアウトで失敗する。これは CP としての
設計意図であり修正すべきバグではない。データプレーンの Eventual write は無影響で、
可用性は CRDT 経路が担う。
...
sequenceDiagram
    ...
    N1->>N1: Append entry to Raft log (fsync)

    par Log replication
        N1->>N2: AppendEntries(entry)
        N2->>N2: fsync log
        N2-->>N1: ack (match_index)
        N1->>N3: AppendEntries(entry)
        N3->>N3: fsync log
        N3-->>N1: ack (match_index)
    end

    N1->>N1: Majority match -> commit, apply to system namespace
    N1-->>Admin: 200 OK (commit-ordered policy version)

    Note over N2,N3: Followers apply on next leader_commit advance
```

リーダー以外のノードが PUT/DELETE を受けた場合は `503 NOT_LEADER` と
`x-asteroidb-leader-id` / `x-asteroidb-leader-addr` ヒントヘッダを返し、
クライアントがリーダーへ再送する（サーバー側転送は行わない）。
...
| System namespace を DB 自身に | 外部調整サービス（例: etcd）が不要。Control plane は内蔵 Raft で自身の設定を合意・複製する。 |
| Control plane に素の Raft（内蔵） | 対称 majority quorum + ランダム化タイムアウト。term = フェンシングトークン。低頻度・小ログなのでスループット最適化は不要で、正しさと理解容易性を優先。メンバーシップは静的（joint consensus はスコープ外）。クラッシュ故障耐性には十分。Byzantine 耐性は将来対応。 |
