...
> 再定義: (1) 合意フットプリントを「まれにしか変わらない小さな状態」(roster + policy +
> authority 名簿版 — 同文書 §3.3 のラウンド 1 改訂で名簿が版付き一次データに昇格)に
...
現実装ではまだ実現されていない(全ノード全キー複製 + namespace 全複製)。さらに
`docs/design/core-semantics-v2.md` §3.2 の coverage 証明は、正直な証明の対価として
**O(origins) の報告メタデータ**を要する — この主張との緊張は `design/principles.md` の
緊張表 **T3** に登録済み(設計検証ラウンド 3 で v2 §1 例外台帳から移管。origins は
roster で有界化されず、引退は分断中停止する — 弱化も同行が文書化)で、緩和は損失なしの
転送工学(レーン分離・差分報告・origin 引退)による。
また authority 名簿は同文書 §3.3 により **Raft コミット対象の版付き一次データ**に
昇格する(certification 分母の pin — 合意フットプリントは「roster + policy + 名簿版」)。
到達には二本柱が要る:
