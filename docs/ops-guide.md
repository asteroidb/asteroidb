...
| GET | `/api/eventual/read/:key` | Eventual 読み取り |
| POST | `/api/certified/write` | Certified 書き込み |
| GET | `/api/certified/read/:key` | Certified 読み取り |
| GET | `/api/certified/status/:key` | Certification ステータス |
| POST | `/api/certified/verify` | 証明検証 |
| GET | `/api/version-history/:key` | バージョン履歴 |
...
| DELETE | `/api/control-plane/policies` | 配置ポリシー削除 |
