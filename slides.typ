...
#set page(paper: "presentation-16-9", margin: (x: 2cm, y: 1.5cm))
#set text(font: "Noto Sans CJK JP", size: 18pt, lang: "ja")
...
#let title-slide(title, subtitle: none) = {
  ...
  block(width: 100%)[
    ...
    #if subtitle != none {
      v(0.5em)
      text(size: 22pt, fill: rgb("#555"))[#subtitle]
    }
  ]
}
...
#rect(fill: rgb("#f0f4ff"), radius: 6pt, inset: 1em, width: 100%)[
  *HlcTimestamp の構造:*

  #align(center)[
    `(physical_ms: u64, logical: u32, node_id: NodeId)`
  ]
...
#rect(fill: rgb("#fff8e8"), radius: 6pt, inset: 1em, width: 100%)[
...
*ポイント:*
- 各ノードは自分のカウンタのみ更新 → 衝突なし
- `max` による merge は冪等・可換・結合 ✓
...
#rect(fill: rgb("#f0f4ff"), radius: 6pt, inset: 1em, width: 100%)[
  *データ構造:*
  - `value: Option<T>` — 現在の値
  - `timestamp: HlcTimestamp` — 最終書き込み時刻
]
...
*操作:*
- `add(elem)`: 新しいドットを生成し `entries[elem]` に追加
  ```
  counter[self] += 1
  entries[elem].insert((self, counter[self]))
  ```
- `remove(elem)`: 現在のドットをすべて `deferred` に移動し、`entries` から削除
  ```
...
fn merge(other):
  for key in all_keys:
    ...
    // 3. キーが存在する場合のみ保持
    if kept_dots.is_empty():
      entries.remove(key)
    else:
      entries[key] = (kept_dots, merged_value)
```
