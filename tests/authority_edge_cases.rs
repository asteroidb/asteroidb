...
#[test]
fn residual_frontier_does_not_certify_an_emptied_authority_set() {
    let mut ns = SystemNamespace::new();
    ns.set_placement_policy(PlacementPolicy::new(PolicyVersion(1), kr(""), 1).with_certified(true))
        .unwrap();
    ...
    // A frontier admitted while the authority set was still populated. It
    // survives the set being emptied: `set_authority_definition` does not bump
    // the placement policy version, so the frontier stays in scope.
    api.update_frontier(AckFrontier {
    ...
    // Whether or not the write is admitted, nothing may reach Certified.
    let _ = api.certified_write("key-a".into(), counter_value(1), OnTimeout::Pending);
    api.process_certifications();

    assert!(
        api.pending_writes()
            .iter()
            .all(|pw| pw.status != CertificationStatus::Certified),
        "a single residual frontier must not certify a write against an \
         empty authority set"
    );
}
