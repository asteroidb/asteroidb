...
- **認可**: 有効なトークンを持つリクエストは control plane を変更可能。
  MVP ではロールベースアクセス制御なし。変更は内蔵 Raft コンセンサスで
  合意される（リーダーのみ受理、静的投票者集合の過半数 commit）。
  `/api/internal/raft/*` RPC も同じ Bearer トークンで保護される —
  **トークン未設定時は無認証**のため、本番ではトークン設定を必須とする
  （`docs/ops-guide.md` §14.6 参照）。
...
- **Keyset バージョニング**: 鍵は単調増加する `keyset_version`（初期値 1）で
  管理。epoch カウンタ（デフォルト 24 時間）は証明検証時の keyset 失効判定
  （現在 + 過去 7 epoch の猶予）に使われる。
- **自動ローテーションは未配線**: `EpochManager` は存在するが、production で
  `stage_keys` を呼ぶ経路が無く `check_and_rotate` は常に no-op。epoch 境界での
  自動切り替え・猶予期間による自動フォールバックは**発生しない**。
  `keyset_version` は初期値のまま固定される。
- **実際のローテーション手順**: `ASTEROIDB_AUTHORITY_KEYS` の再配布 +
  ローリング再起動による手動手順のみ（`docs/runbook/key-rotation.md` 参照）。
- **鍵配布の信頼モデル**: 鍵配布・ローテーションは env 配布経路
  （構成管理・シークレット配布）の運用信頼に依存する。透明性レイヤ
  （equivocation / split-view 検知）は frontier 報告の一致性のみを覆い、
  **keyset 自体の split-view は検知しない**（下記「既知の制限事項」参照）。
...
7. **鍵配布の split-view は検知の死角（脅威モデル境界）**: equivocation /
   split-view 検知（透明性レイヤ）が覆うのは **frontier 報告の一致性のみ**。
   keyset 履歴に STR（Signed Tree Root）チェーンが無いため、**鍵配布・
   ローテーション自体の split-view**——異なるノードに異なる
   `ASTEROIDB_AUTHORITY_KEYS`（別のキーセット）を配り、各ノードに別々の
   「正しい Authority 鍵」を信じさせる攻撃——は本機構では検知できない。
   keyset の完全性は env 配布経路の運用信頼（構成管理の完全性・配布の
   一貫性検証）に依存する。keyset 履歴への STR チェーン導入は次期課題。
